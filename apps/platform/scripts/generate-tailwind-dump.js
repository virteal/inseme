...
try {
  console.log("Running Tailwind CLI to generate dump...");
  // Prefer locally installed tailwindcss binary if available (faster, avoids npx issues)
  ...
  if (tailwindBin) {
    ...
    if (r.error || r.status !== 0) {
      ...
      let tailwindBin2 = null;
      for (const b2 of possibleBins) {
        try {
          if (fs.existsSync(b2)) {
            tailwindBin2 = b2;
            break;
      ...
      if (tailwindBin2) {
        const args = ["-i", tmpInput, "-o", outDump, ...contentArgs, "--minify"];
        const run2 = spawnSync(tailwindBin2, args, { stdio: "inherit" });
