...
import { getSupabase, getInstance, getSubdomain } from "@inseme/cop-host";

// On simule getConfig car wikiFederation l'utilisait via getFederationConfig
...
function getLocalSubdomainSafe() {
  ...
  // fallback to getFederationConfig
  ...
  while (depth < maxDepth) {
    ...
    if (found) {
      return {
        page: found,
        hubInfo,
        resolvedFrom: depth === 0 ? "local" : "parent",
        chain,
      };
    }
    // Move to parent
    ...
    try {
      ...
      hubInfo = {
        url: parent.url,
        subdomain: parent.url.replace(/https?:\/\//, ""),
      };
    } catch (e) {
...
export async function upsertLocalPage({
  ...
  const isGlobalRoot =
    federationCfg?.hubType === "national" || federationCfg?.isHub === true;
  ...
  if (parent_revision_global_id)
    wikiMeta.parent_revision_global_id = parent_revision_global_id;
...
export async function archiveLocalPageIfAdoptedUpstream({ pageKey }) {
  ...
  try {
    ...
    if (!localGlobalId)
      return { success: false, reason: "no global id on local" };
    ...
    const { data: parentPage } = await parentClient
      ...
      .select("*")
      .filter(
        "metadata->wiki_page->>parent_revision_global_id",
        "eq",
        localGlobalId
      )
      .limit(1)
    ...
    console.error(
      "archiveLocalPageIfAdoptedUpstream error",
      err.message || err
    );
    return { success: false, error: err.message || String(err) };
  }
...
export async function proposeToParent({
  pageKey,
  notifyParent = false,
  parentApiKey = null,
}) {
  ...
  try {
    ...
    if (notifyParent) {
      ...
      if (parent?.url && parentApiKey) {
        try {
          const remoteClient = await createRemoteClientForUrl(
            parent.url,
            parentApiKey
          );
          const payload = {
            ...
            metadata: {
              wiki_page: {
                ...
                parent_revision_global_id:
                  local.metadata?.wiki_page?.global_id || null,
