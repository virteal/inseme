...
import { Card, Button, Badge, Progress, Avatar } from "../../../ui/src/index";
import { useInseme } from "../../../inseme-core/index";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-slate-900/50 border-slate-800 p-6 backdrop-blur-sm h-[500px] flex flex-col">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <MessageSquare className="w-5 h-5 text-purple-400" /> Live Macagna
              & Chat
            </h2>
            <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
              {messages?.slice(-20).map((msg) => (
                <div
                  key={msg.id}
                  className={`flex gap-3 ${msg.type === "ai" ? "bg-amber-500/5 p-3 rounded-xl border border-amber-500/10" : ""}`}
                >
