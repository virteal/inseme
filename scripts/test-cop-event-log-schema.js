...
const migrationPath = path.join(process.cwd(), "apps/platform/supabase/migrations/20260731180000_cop_append_only_event_log.sql");
...
assert.ok(sqlContent.includes("CREATE TABLE IF NOT EXISTS public.github_webhook_deliveries"), "Missing github_webhook_deliveries table");
assert.ok(sqlContent.includes("CREATE TABLE IF NOT EXISTS public.cop_event_log"), "Missing cop_event_log table");
assert.ok(sqlContent.includes("CREATE TABLE IF NOT EXISTS public.cop_spool_queue"), "Missing cop_spool_queue table");
...
assert.ok(sqlContent.includes("enforce_cop_event_log_immutability"), "Missing immutability trigger function");
assert.ok(sqlContent.includes("BEFORE UPDATE OR DELETE ON public.cop_event_log"), "Immutability trigger must intercept UPDATE and DELETE");
...
assert.ok(sqlContent.includes("CONSTRAINT cop_event_log_topic_seq_uniq UNIQUE (topic_id, topic_seq)"), "Missing UNIQUE(topic_id, topic_seq) constraint");
...
assert.ok(sqlContent.includes("ALTER TABLE public.cop_event_log ENABLE ROW LEVEL SECURITY"), "RLS must be enabled on cop_event_log");
