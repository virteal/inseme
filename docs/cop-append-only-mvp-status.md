...
Commits of record: `91b9141`, `e5cec1d`, `9f749ed`.

## Residual checklist — #28 (keep OPEN)

- [ ] COP/Core **storage interfaces** independent of Supabase (types + ports)
- [ ] **Working** SQLite/NDJSON spool + **replay** (table alone is not enough)
- [ ] Versioned **`cop.event/v1` JSON Schema** + runtime validation of full envelope
- [ ] Store-level tests: topic order, duplicate ingestion, rejected UPDATE/DELETE
- [ ] Export/import path
- [ ] Artifact externalization (hash + object-store ref) + restricted visibility tests
- [ ] Short note: source vs artifact vs projection vs cache vs export
